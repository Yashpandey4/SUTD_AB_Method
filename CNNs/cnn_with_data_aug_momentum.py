...
optimizer = tf.train.MomentumOptimizer(learning_rate, FLAGS.momentum,use_nesterov=True).minimize(cost+ l2 * FLAGS.weight_decay)
    ...
    for epoch in range(epochs):
        ...
        for batch_i in range(1, n_batches + 1):
            for batch_features, batch_labels in load_preprocess_training_batch(batch_i, batch_size):
                batch_features = data_augmentation(batch_features)
                #Train C
