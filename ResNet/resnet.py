...
class model():
    ...
    def __init__(self):
        self.fc_w1 = self.create_variables(name='fcl1_weights', shape=[64, 64], is_fc_layer=True,
                                initializer=tf.contrib.layers.xavier_initializer())
        self.fc_b1 = self.create_variables(name='fcl1_bias', shape=[64], initializer=tf.zeros_initializer())

        self.fc_w2 = self.create_variables(name='fcl2_weights', shape=[64, 64], is_fc_layer=True,
                                initializer=tf.contrib.layers.xavier_initializer())
        self.fc_b2 = self.create_variables(name='fcl2_bias', shape=[64], initializer=tf.zeros_initializer())
    ...
    def inference(self, input_tensor_batch, n, reuse):
        ...
        with tf.variable_scope('fc', reuse=reuse):
            ...
            fc_h1 = tf.nn.relu(tf.matmul(global_pool, self.fc_w1) + self.fc_b1) + global_pool
            fc_h1d = tf.nn.dropout(fc_h1, pkeep) * pkeep
            layers.append(fc_h1d)

            fc_h2 = tf.nn.relu(tf.matmul(fc_h1d, self.fc_w2) + self.fc_b2) + fc_h1
            fc_h2d = tf.nn.dropout(fc_h2, pkeep) * pkeep
            layers.append(fc_h2d)

            output = self.output_layer(fc_h2d, 10)
            layers.append(output)

        return layers[-1] # returns or last element of the list
    ...
    def test_graph(self, train_dir='logs'):
        ...
        result = self.inference(input_tensor, 2, reuse=False)
