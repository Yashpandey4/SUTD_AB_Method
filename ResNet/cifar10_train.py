...
class Train(object):
    ...
    def build_train_validation_graph(self, modelname, str):
        '''
        ...
        with tf.variable_scope(str):
            logits = modelname.inference(self.image_placeholder, FLAGS.num_residual_blocks, reuse=False)
            vali_logits = modelname.inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse=True)
    ...
    def train(self):
        ...
        self.build_train_validation_graph(C,"C")
        ...
        trans_arr1 = tf.Variable((np.random.choice([0., 1.], size=(64,64), p=[1-ptrans, ptrans])), dtype=tf.float32)  # 784 = 28 * 28
        trans_arr2 = tf.Variable((np.random.choice([0., 1.], size=(64,64), p=[1-ptrans, ptrans])), dtype=tf.float32)

        ass1=(A.fc_w1.assign(tf.add(tf.multiply(C.fc_w1, trans_arr1),tf.multiply(A.fc_w1,tf.subtract(tf.ones([64,64]),trans_arr1)))))
        ass2=(B.fc_w1.assign(tf.add(tf.multiply(B.fc_w1, trans_arr1),tf.multiply(C.fc_w1,tf.subtract(tf.ones([64,64]),trans_arr1)))))
        ass3=(A.fc_w2.assign(tf.add(tf.multiply(C.fc_w2, trans_arr2),tf.multiply(A.fc_w2,tf.subtract(tf.ones([64,64]),trans_arr2)))))
        ass4=(B.fc_w2.assign(tf.add(tf.multiply(B.fc_w2, trans_arr2),tf.multiply(C.fc_w2,tf.subtract(tf.ones([64,64]),trans_arr2)))))

        a1=C.fc_w1.assign(tf.add(tf.multiply(A.fc_w1, trans_arr1),tf.multiply(B.fc_w1,tf.subtract(tf.ones([64,64]),trans_arr1))))
        a2=C.fc_w2.assign(tf.add(tf.multiply(A.fc_w2, trans_arr2),tf.multiply(B.fc_w2,tf.subtract(tf.ones([64,64]),trans_arr2))))
        ...
        for step in range(FLAGS.train_steps):
            ...
            sess.run([ass1,ass2,ass3,ass4])

            # Update transfer arrays
            trans_arr1.load((np.random.choice([0., 1.], size=(64,64), p=[1-ptrans, ptrans])),sess)  # 784 = 28 * 28
            trans_arr2.load((np.random.choice([0., 1.], size=(64,64), p=[1-ptrans, ptrans])),sess)
            ...
            if step % FLAGS.report_freq == 0:
                ...
                epoch = math.ceil(step * FLAGS.train_batch_size / 60000.)

                format_str = ('%s: epoch %d, step  %d, loss = %.4f (%.1f examples/sec; %.3f ' 'sec/batch)')
                print (format_str % (datetime.now(), epoch, step, train_loss_value, examples_per_sec,
                                    sec_per_batch))
                print ('Train accuracy = %8.5f' % ((1-train_error_value)*100))
                # print ('Train top1 error = ', train_error_value)
                print ('Validation accuracy = %5.2f' % ((1-validation_error_value)*100))
                ...
                if(epoch>=50):
                    ...
                    H.fc_w1=ptrans*A.fc_w1+(1-ptrans)*B.fc_w1
                    H.fc_w2=ptrans*A.fc_w2+(1-ptrans)*B.fc_w2
                    H.fc_b1=ptrans*A.fc_b1+(1-ptrans)*B.fc_b1
                    H.fc_b2=ptrans*A.fc_b2+(1-ptrans)*B.fc_b2

                    with tf.variable_scope("A"):
                        logits = A.inference(self.image_placeholder, FLAGS.num_residual_blocks, reuse=tf.AUTO_REUSE)
                        vali_logits = A.inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse=True)
                        if FLAGS.is_full_validation is True:
                                    validation_loss_value, validation_error_value = self.full_validation(loss=self.vali_loss,
                                                            top1_error=self.vali_top1_error, vali_data=vali_data,
                                                            vali_labels=vali_labels, session=sess,
                        ...
                        val_error_list_A.append(validation_error_value)

                        print ('A : Validation accuracy = %5.2f' % ((1-validation_error_value)*100))
                        # print ('Validation top1 error = %.4f' % validation_error_value)
                        print ('A : Validation loss = ', validation_loss_value)              
    
                    with tf.variable_scope("B"):
                        logits = B.inference(self.image_placeholder, FLAGS.num_residual_blocks, reuse=tf.AUTO_REUSE)
                        vali_logits = B.inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse=True)
                        if FLAGS.is_full_validation is True:
                                    validation_loss_value, validation_error_value = self.full_validation(loss=self.vali_loss,
                                                            top1_error=self.vali_top1_error, vali_data=vali_data,
                                                            vali_labels=vali_labels, session=sess,
                        ...
                        val_error_list_B.append(validation_error_value)

                        print ('B : Validation accuracy = %5.2f' % ((1-validation_error_value)*100))
                        # print ('Validation top1 error = %.4f' % validation_error_value)
                        print ('B : Validation loss = ', validation_loss_value)
            
                    with tf.variable_scope("H"):
                        logits = H.inference(self.image_placeholder, FLAGS.num_residual_blocks, reuse=tf.AUTO_REUSE)
                        vali_logits = H.inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse=True)
                        if FLAGS.is_full_validation is True:
                                    validation_loss_value, validation_error_value = self.full_validation(loss=self.vali_loss,
                                                            top1_error=self.vali_top1_error, vali_data=vali_data,
                                                            vali_labels=vali_labels, session=sess,
                        ...
                        val_error_list_H.append(validation_error_value)

                        print ('H : Validation accuracy = %5.2f' % ((1-validation_error_value)*100))
                        # print ('Validation top1 error = %.4f' % validation_error_value)
                        print ('H : Validation loss = ', validation_loss_value)
        ...
        print()
        print("C : Min Validation Error : %1.5f" % min(val_error_list))
        print()
        print("A : Min Validation Error : %1.5f" % min(val_error_list_A))
        print()
        print("B : Min Validation Error : %1.5f" % min(val_error_list_B))
        print()
        print("H : Min Validation Error : %1.5f" % min(val_error_list_H))
