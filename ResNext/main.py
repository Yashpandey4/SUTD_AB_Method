...
parser.add_argument('--alpha', type=float, default=0.001, help='coefficent of new reguralization')
...
def main():
  ...
  for epoch in range(args.start_epoch, args.epochs):
    ...
    dummy = recorder.update(epoch, train_los, train_acc, val_los, val_acc)
...
def train(train_loader, model, criterion, optimizer, epoch, log):
  ...
  for i, (input, target) in enumerate(train_loader):
    ...
    reg = torch.max(torch.sum(torch.abs(output), dim=0))
    total_loss = loss + args.alpha * reg
...
def accuracy(output, target, topk=(1,)):
  ...
  for k in topk:
    ...
    res.append(correct_k.mul_(100.0 / batch_size).detach())
  return res
